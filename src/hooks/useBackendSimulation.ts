...
export function useBackendSimulation() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      // Fetch blackout simulation status
      const blackoutResponse = await fetch('http://localhost:8001/api/blackout/status');
      if (blackoutResponse.ok) {
        const blackoutData = await blackoutResponse.json();
        // We'll handle this in the Index component, but we could expose it here if needed
      }

      // Fetch telemetry
      const telemetryResponse = await fetch('http://localhost:8001/api/telemetry/latest');
      if (telemetryResponse.ok) {
        ...
        setState(prev => ({ 
          ...
          telemetry: {
            ...telemetryData,
            // Ensure actual_payload is included
            actual_payload: telemetryData.actual_payload || null
          }
        }));
      }

      // Fetch link status
      const linkResponse = await fetch('http://localhost:8001/api/link/status');
      if (linkResponse.ok) {
      ...
      const packetsResponse = await fetch('http://localhost:8001/api/packets/stored');
      ...
      const systemResponse = await fetch('http://localhost:8001/system/status');
  ...
  const toggleRunning = useCallback(async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8001/api/mission/${endpoint}`, {
        method: 'POST',
      });
      if (response.ok) {
  ...
  const toggleOrbitnet = useCallback(async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8001/system/mode/${newMode}`, {
        method: 'POST',
      });
      if (response.ok) {
  ...
  const reset = useCallback(async () => {
    try {
      const response = await fetch('http://localhost:8001/api/mission/reset', {
        method: 'POST',
      });
      if (response.ok) {
        setState(prev => ({
